...
var log = require('d1-logger');

/**
 * DockerCompose is a Javascript wrapper of Docker Compose HTTP API.
 * Usage: {@link DockerCompose}.{@link putInitParams()}.{@link putEnv()}.{@link pseudOperation()} where pseudOperations can be {@link up()} or any other docker compose operations described in this class. Options for pseudoOperations can be referenced at Docker homepage or http://blog.schoolofdevops.com/wp-content/uploads/2016/10/Docker-compose-Cheat-Sheet-V1-1.pdf
 * @constructor
 */
...
DockerCompose.prototype._execute = function (command, opts, services, sub_opts) {
	...
	return new Promise((resolve, reject) => {
        ...
        log.debug(`env: ${JSON.stringify(this.env)}`)
        log.debug(`docker-compose ${args.join(' ')}`)
