...
DockerHandler.prototype.copyDockerFiles = function (container, src, dst_path, dst_file) {
	...
	let dst = dst_path.endsWith('/') ? [dst_path, Math.random()].join('') : [dst_path, Math.random()].join('/')
	return new Promise.try( _ => container.getArchive({ path: src }) ) // Getting tarball of source file from container
		.then(incomming_message => {
			...
			// debug(`incomming_message = ${inspect(incomming_message)}`) // incomingmessage
			// return this._writeStream(incomming_message, dst) // WORKING FOR ALL ONE OR THREE COPY FUNCTIONS
			return this._writeStream(incomming_message, dst_path, dst_file) // --- MISSING ONE FILE IN CASE THREE, FINE IN CASE ONE
		})
		.then(_ => {
			return tar.x(
				{
					cwd: dst_path,
					file: dst_path + '/' + dst_file + '.tar'
				}
			).then(_ => {
				debug(`extracted ${dst_file}`)
			})
		})

}

DockerHandler.prototype._writeStream = function (stream, path, file) {
	debug(`Streaming file ${file}.tar`)
	let wfile = path + '/' + file + '.tar'
	return Promise.try( _ => fs.mkdirsAsync(path))
		.then( _ => fs.openAsync(wfile, 'w') )
		.then( _ => fs.createWriteStream(wfile) )
		.then( writable => {
			return new Promise((resolve, reject) => {
...
//             // debug(`incomming_message = ${inspect(incomming_message)}`) // incomingmessage
//             // return this._writeStream(incomming_message, dst) // WORKING FOR ALL ONE OR THREE COPY FUNCTIONS
//             return this._writeStream(incomming_message, dst_path) // --- MISSING ONE FILE IN CASE THREE, FINE IN CASE ONE
// 		})
//
// }
//
...
// 		})
//
// 		stream.on('finish', _ => {debug(`compressDir ${src} to ${dst} finished`)} )
//
// 		stream.on('close', _ => {
// 			debug(`Stream closed. Resolved`)
// 			resolve()
