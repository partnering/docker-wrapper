...
DockerHandler.prototype.inspectContainerStatus = function (name) {
	return Promise.try( _ => this.inspectContainer(name) )
		...
		.catch( err => {
			log.warn(`Container ${name} inspection fails: ${err.message}`)
			return {
				State: {
					Running: false,
					Status: 'non-existent',
...
DockerHandler.prototype.disconnectContainerFromNetwork = function (container, network) {
	return new Promise((resolve, reject) => {
		network.disconnect({
			...
			if (!!err) {
				log.warn(`Disconnecting network and container ${container.Id}: failed. Error: ${err.message}`)
				reject(err)
				return
			}
			debug(`Disconnecting network and container ${container.Id}: success`)
			resolve(data)
		})
	})
}
...
DockerHandler.prototype.findExistingVolumes = function (volumes) {
	...
	return Promise.try( _ => this.docker.listVolumes(params) )
		.then( data => data == null ? null : data.Volumes )
		.catch( err => {
			log.warn(`Searching for volumes ${inspect(volumes)} fails. Error: ${err.message}`)
			throw err
		})
}
...
DockerHandler.prototype.doesContainerExist = function (id) {
	return this.inspectContainer(id)
		.then(_ => true)
		.catch(err => {
			log.warn(`Container ${id} inspection fails. Error: ${err.message}`)
			return false
		})
}
...
DockerHandler.prototype.createOverlayNetwork = function (id) {
	return Promise.try(_ => this.doesNetworkExist(id))
		...
		.catch(err => {
			log.warn(`Network ${id} creation fails. Error: ${err.message}`)
			throw err
		})
}
