...
DockerHandler.prototype.listNetworks = function () {
    return Promise.promisify(this.docker.listNetworks).bind(this.docker)({})
        .then(netws => {
            let networks
            if (netws != null)  {
                networks = netws.map( netw => {
                    let obj = this.getContainer(netw.Id);
                    for (let prop in netw) {
                        obj[prop] = netw[prop];
                    }
                    return obj;
                })
...
DockerHandler.prototype.getNetworkByName = function (name) {
    return this.listNetworks()
        ...
        .then(results => {
            debug(`Found ${!!results ? results.length : -1} networks named ${name}`)
...
DockerHandler.prototype.listContainersByName = function (name) {
    return Promise.promisify(this.docker.listContainers).bind(this.docker)
        ...
        .then(results => {
            debug(`Found ${results.length} containers named ${name}`)
            return results
        })
        .catch(err => {
...
DockerHandler.prototype.disconnectContainerFromNetwork = function (container, network) {
    return new Promise((resolve, reject) => {
        debug(`disconnectContainerFromNetwork: network = ${inspect(network)}`)
        network.disconnect({
...
DockerHandler.prototype.createOverlayNetwork = function (id) {
    return Promise.try( _ => this.networkExists(id) )
        .then( exist => {
            if (exist) {
