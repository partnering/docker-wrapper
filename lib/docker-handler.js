			...
			exec.start((err, stream) => {
				...
				stream.on('end', _ => {
					exec.inspect((err, data) => {
						if (err != null) {
							log.error(`Exec ${cmd} inspecting failed: ${err.message}`)
							reject(err);
							return
						}
						if (data == null || data.Running || data.ExitCode != 0) {
							log.error(`Exec ${cmd} failed. Unexpected returned data: ${inspect(data)} `)
							reject(err)
							return
						}
						resolve(data)
...
/**
 * Retrieve directory of volume mounted to container
 * @param {string} container_name
 * @param {string} volume_name
 * @returns {string} directory to the volume
 */
DockerHandler.prototype.findVolumeDirectory = function (container_name, volume_name) {
    return Promise.try(_ => this.inspectContainer(container_name))
        .then(res => {
			debug(`inspection of ${container_name}, ${volume_name}`, res['Mounts'])
            return res['Mounts'].find(mount => mount.Type === 'volume' && mount.Name === volume_name)
        })
        .then(res => {
            debug(`Found mount`, res)
            return res == null ? null : res['Source']
        })
}
 ...
 * @param {Function} [handler] - optional handler to send build messages to terminal
 * @return {Promise.<>}
 */
DockerHandler.prototype.buildImage = function (file, opts, handler) {
	return new Promise((resolve, reject) => {
		this.docker.buildImage(file, opts, (err, stream) => {
			...
			stream.on('data', chunk => {
				...
				if (json == null || json.stream == null) return
